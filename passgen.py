...
if __name__ == '__main__':
    ...
    for row in range(int(rows)-2):
        ...
        passwdString = ''.join( [chr(random.randint(33,126)) for i in range(0,int(passwdSize))] )
    ...
    pyperclip.copy(''.join( [chr(random.randint(1,31)) for i in range(0,len(passwdArray[-1]))] ))
